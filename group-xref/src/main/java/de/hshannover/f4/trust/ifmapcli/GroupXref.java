 ...
 * 
 * This file is part of ifmapcli (group-xref), version 0.3.1, implemented by the Trust@HsH
...
/**
 * A simple tool that publishes or deletes group-xref metadata.<br/>
 * When metadata is published, the lifetime is set to be 'forever'.
 *
 * @author pe
 *
 */
public class GroupXref extends AbstractClient {
	...
	public static void main(String[] args) {
		...
		printParameters(KEY_OPERATION, new String[] {KEY_ICS_OVERLAY_MANAGER_GROUP, KEY_ICS_LDAP_URI});
		...
		// prepare metadata
		String ldapUri = resource.getString(KEY_ICS_LDAP_URI);

		Document metadata = icsmf.createGrpXref(ldapUri);
		...
		if (isUpdate(KEY_OPERATION)) {
			...
			String filter = "ics-meta:group-xref";
			PublishDelete publishDelete = Requests.createPublishDelete(ovManGrIdentifier, filter);
