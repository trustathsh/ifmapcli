...
import de.hshannover.inform.trust.ifmapvaliditychecker.IfmapValidityChecker;
import de.hshannover.inform.trust.ifmapvaliditychecker.check.CheckResult;
import de.hshannover.inform.trust.ifmapvaliditychecker.exceptions.ValidityCheckerException;

/**
 * A tool that does an IF-MAP search and uses the ifmapvaliditychecker
 * library to validate. The result of the validation is the printed on 
 * the command line.
 * 
 * Command line arguments specify the search parameters as well as the
 * filenames of the XML file that contains the rules for validation
 * and the corresponding XSD schema.
 * 
 ...
 * 
 */
public class ValidityTool {
	final static String CMD = "validity-tool";
	...
	Option mFilenameRules;
	Option mFilenameSchema;
	 ...
	 */
	public ValidityTool(String[] args) throws FileNotFoundException,
	...
	/**
	 * Fetch {@link ResultItem} objects and validate them by IfmapValidityChecker
	 */
	private void validateSearchResult() {
		...
		// set path/filename of rules.xml
		if (mCmdLine.hasOption(mFilenameRules.getOpt())) {
			filenameRules = mCmdLine.getOptionValue(mFilenameRules.getOpt());
		}
		
		// set path/filename of schema.xsd
		if (mCmdLine.hasOption(mFilenameSchema.getOpt())) {
			filenameSchema = mCmdLine.getOptionValue(mFilenameSchema.getOpt());
		}

		try {
			Collection<CheckResult> checkResults = IfmapValidityChecker.check(mSearchResult, filenameRules, filenameSchema);
...
//		OptionBuilder.withLongOpt("value");
		...
		// filename of Rules.xml
		OptionBuilder.hasArg();
		OptionBuilder.isRequired();
		OptionBuilder.withDescription("filename (with path) of Rules XML. example: /home/test/Rules.xml");
//		OptionBuilder.withLongOpt("filename-rules");
		mFilenameRules = OptionBuilder.create("fR");
		mOptions.addOption(mFilenameRules);
		
		// filename of Schema.xsd
		OptionBuilder.hasArg();
		OptionBuilder.isRequired();
		OptionBuilder.withDescription("filename (with path) of Schema XSD. example: /home/test/Schema.xsd");
//		OptionBuilder.withLongOpt("filename-schema");
		mFilenameSchema = OptionBuilder.create("fS");
		mOptions.addOption(mFilenameSchema);
...
//		OptionBuilder.withLongOpt("namespace-uri");
		...
		try {
			...
			formatter.printHelp(
					ValidityTool.CMD + " -i <type> -v <value> -fR <path and filename of rules.xml> - fS <path and filename of schema.xsd> [OPTION]...", mOptions);
	...
	public static void main(String[] args) {
		try {
			ValidityTool v = new ValidityTool(args);
			v.start();
