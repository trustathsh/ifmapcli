 ...
 * 
 * This file is part of ifmapcli (ar-mac), version 0.0.6, implemented by the Trust@HsH
...
/**
 * A simple tool that publishes or deletes access-request-mac metadata.<br/>
 ...
 */
public class ArMac {
	final static String CMD = "ar-mac";
	final static int MIN_ARGS = 3;			// update|delete, ar, mac,
	final static int EXPECTED_ARGS = 8;		// update|delete, ar, mac,
											// url, user, pass,
	...
	public static void main(String[] args) {
		...
		// prepare metadata
		metadata = mf.createArMac();

		// update or delete
		if(Common.isUpdate(op)){
			publishUpdate = Requests.createPublishUpdate(arIdentifier, macIdentifier,
					metadata, MetadataLifetime.forever);
			req = Requests.createPublishReq(publishUpdate);
		} else {
			publishDelete = Requests.createPublishDelete(arIdentifier, macIdentifier, "meta:access-request-mac");
	...
	private static void usage() {
		System.out.println("usage:\n" +
				"\t" + ArMac.CMD + " update|delete ar mac " +
