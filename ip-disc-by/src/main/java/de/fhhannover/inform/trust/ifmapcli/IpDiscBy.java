 ...
 * between ip and device identifiers.<br/>
 * When metadata is published, the lifetime is set to be 'forever'.
 * 
 * @author ib
 *
 */
public class IpDiscBy {
	final static String CMD = "ip-disc-by";
	final static int MIN_ARGS = 3;			// update|delete, ip, dev
	final static int EXPECTED_ARGS = 8;		// update|delete, ip, dev
											// url, user, pass,
	...
	public static void main(String[] args) {
		...
		if(Common.isUpdate(op)){
			publishUpdate = Requests.createPublishUpdate(ipIdentifier, devIdentifier,
					metadata, MetadataLifetime.forever);			
			req = Requests.createPublishReq(publishUpdate);
		} else {
			String filter = "meta:discovered-by";
			publishDelete = Requests.createPublishDelete(ipIdentifier, devIdentifier, filter);
	...
	private static void usage() {
		System.out.println("usage:\n" +
				"\t" + IpDiscBy.CMD + " update|delete ip dev " +
