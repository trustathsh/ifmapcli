 ...
 */
public class ReqInv {
	final static String CMD = "req-inv";
	final static int MIN_ARGS = 4;			// update|delete, dev, ip
	final static int EXPECTED_ARGS = 9;		// update|delete, dev, ip,
											// url, user, pass,
	...
	public static void main(String[] args) {
		...
		// prepare metadata
		metadata = mf.createRequestForInvestigation(qualifier);
		
		// update or delete
		if(Common.isUpdate(op)){
			publishUpdate = Requests.createPublishUpdate(devIdentifier, ipIdentifier,
					metadata, MetadataLifetime.forever);			
			req = Requests.createPublishReq(publishUpdate);
		} else {
			String filter = "meta:request-for-investigation";
			publishDelete = Requests.createPublishDelete(devIdentifier, ipIdentifier, filter);
	...
	private static void usage() {
		System.out.println("usage:\n" +
				"\t" + ReqInv.CMD + " update|delete dev ip qualifier" +
